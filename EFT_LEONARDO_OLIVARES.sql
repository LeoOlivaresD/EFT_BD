...
/***************************************************************************************************
    INFORME 1 (En esta caso no interepete que en el enunciado se 
    debiesen excluir las regiones que no contarancon un cliente de 20 o mas años de antiguedad
****************************************************************************************************/
--LIMPIO VISTA
DROP VIEW v_resumen_clientes_region;

-- Creo la vista v_resumen_clientes_region para almacenar la consulta.
CREATE OR REPLACE VIEW v_resumen_clientes_region AS

-- Selecciono el nombre de la región, la cantidad de clientes con más de 20 años de inscripción y el total de clientes por región.
SELECT
    r.nombre_region AS REGION,
    COUNT(CASE WHEN MONTHS_BETWEEN(SYSDATE, c.fecha_inscripcion) / 12 >= 20 THEN 1 END) AS CLIENTES_MAS_20_ANIOS,
    COUNT(*) AS total_clientes
    
-- Uno las tablas CLIENTE y REGION para obtener la información necesaria.
FROM
    cliente c
INNER JOIN
    region r ON c.cod_region = r.cod_region

-- Agrupo los resultados por región para obtener un resumen por cada una.
GROUP BY
    r.nombre_region

-- Ordeno los resultados por la cantidad de clientes con más de 20 años de inscripción de manera ascendente para mostrar las regiones con menos clientes primero.
ORDER BY
    CLIENTES_MAS_20_ANIOS ASC;

--Revisamos la salida de la vista creada
SELECT * FROM v_resumen_clientes_region;

-- Creo el índice IDX_REGION en la tabla CLIENTE para mejorar el rendimiento de la consulta al acceder a los datos de la región.
CREATE INDEX IDX_REGION ON cliente (cod_region);

-- Creo el índice IDX_CLI_REGION en la tabla CLIENTE para mejorar aún más el rendimiento al acceder a los datos de la región y la fecha de inscripción.
CREATE INDEX IDX_CLI_REGION ON cliente (fecha_inscripcion, cod_region, numrun);
...
-- Selecciono la fecha de hoy, el código de transacción, el nombre del tipo de transacción en mayúsculas y el promedio truncado del monto de la transacción
SELECT
    TO_CHAR(SYSDATE, 'DD-MM-YYYY') AS FECHA,
    tt.cod_tptran_tarjeta AS CODIGO,
    UPPER(tt.nombre_tptran_tarjeta) AS DESCRIPCION,
    TRUNC(AVG(t.monto_transaccion), 0) AS "MONTO PROMEDIO TRANSACCION" -- Cambio aquí: promedio del monto de la transacción
    
-- Desde la tabla TRANSACCION_TARJETA_CLIENTE
FROM
    TRANSACCION_TARJETA_CLIENTE t
    
-- Unida a la tabla TIPO_TRANSACCION_TARJETA
INNER JOIN
    TIPO_TRANSACCION_TARJETA tt ON t.cod_tptran_tarjeta = tt.cod_tptran_tarjeta

-- Unida a la tabla CUOTA_TRANSAC_TARJETA_CLIENTE
INNER JOIN
    CUOTA_TRANSAC_TARJETA_CLIENTE cttc ON t.nro_tarjeta = cttc.nro_tarjeta AND t.nro_transaccion = cttc.nro_transaccion
    
-- Donde el mes de la fecha de vencimiento de la cuota esté entre 6 y 12 (segundo semestre)
WHERE
    EXTRACT(MONTH FROM cttc.fecha_venc_cuota) BETWEEN 6 AND 12
    
-- Agrupando por código de tipo de transacción y nombre de tipo de transacción
GROUP BY
    tt.cod_tptran_tarjeta, tt.nombre_tptran_tarjeta
    
-- Ordenando por monto promedio de la transacción de forma ascendente
ORDER BY
...
*************************************/
--Borro y creo la tabla nuevamente para asegurarme de que no haya ningun error
DROP TABLE seleccion_tipo_transaccion CASCADE CONSTRAINTS;
CREATE TABLE seleccion_tipo_transaccion
(
fecha VARCHAR2(10) NOT NULL,
...
-- Actualizo la tabla TIPO_TRANSACCION_TARJETA para revertir los cambios en la tasa de interés
UPDATE tipo_transaccion_tarjeta
SET tasaint_tptran_tarjeta = 
  CASE cod_tptran_tarjeta  -- utilizo un CASE para actualizar la tasa de interés según el código de transacción
    WHEN 101 THEN 0.03  -- Si el código es 101, la tasa de interés será 0.03
    WHEN 102 THEN 0.04  -- Si el código es 102, la tasa de interés será 0.04
    WHEN 103 THEN 0.15  -- Si el código es 103, la tasa de interés será 0.15
    ELSE tasaint_tptran_tarjeta  -- Si el código es otro, la tasa de interés no se modificará
  END;
 
COMMIT;

-- Inserto datos en la tabla 'seleccion_tipo_transaccion'.
-- Inserto datos en la tabla 'seleccion_tipo_transaccion'.
INSERT INTO seleccion_tipo_transaccion (fecha, cod_tipo_transac, nombre_tipo_transac, monto_promedio)

-- Utilizo Common Table Expression llamada 'Transacciones_Vencimiento_Semestre2' para obtener las transacciones que necesito
WITH Transacciones_Vencimiento_Semestre2 AS (
    SELECT
        TO_CHAR(SYSDATE, 'DD-MM-YYYY') AS FECHA, -- Obtengo la fecha de hoy y la formateo como 'DD-MM-YYYY'
        ttc.cod_tptran_tarjeta AS CODIGO, -- Selecciono el código del tipo de transacción
        UPPER(ttt.nombre_tptran_tarjeta) AS DESCRIPCION, -- Selecciono la descripción del tipo de transacción en mayúsculas
        TRUNC(AVG(ttc.monto_transaccion), 0) AS MONTO -- Calculo el promedio del monto de la transacción y lo trunco a 0 decimales
    ...
    -- Unida a la tabla 'cuota_transac_tarjeta_cliente'
    INNER JOIN
        cuota_transac_tarjeta_cliente cttc ON ttc.nro_tarjeta = cttc.nro_tarjeta AND ttc.nro_transaccion = cttc.nro_transaccion
...
-- Selecciono las columnas que necesito de la CTE 'Transacciones_Vencimiento_Semestre2'
SELECT
    FECHA,
...
FROM
    Transacciones_Vencimiento_Semestre2  -- Uso la CTE como si fuera una tabla

-- Ordeno los resultados por el monto promedio de forma ascendente
ORDER BY
    MONTO_PROMEDIO ASC;
...
-- Muestro la tabla 'seleccion_tipo_transaccion' para verificar los resultados esperados de la imagen del word
SELECT * FROM SELECCION_TIPO_TRANSACCION;


/*************************************************
-- Actualizo la tabla tipo_transaccion_tarjeta
*************************************************/

-- Actualizo la tabla tipo_transaccion_tarjeta ttt
UPDATE tipo_transaccion_tarjeta ttt
SET ttt.tasaint_tptran_tarjeta = TRUNC(ttt.tasaint_tptran_tarjeta * 0.99, 2) -- Rebajo la tasa de interés en un 1% y trunco el resultado a 2 decimales
WHERE EXISTS (
    -- Busco si existe el código de tipo de transacción en la tabla 'seleccion_tipo_transaccion'
    SELECT 1
...
-- Muestro la tabla 'TIPO_TRANSACCION_TARJETA' para verificar los resultados esperados de la figura 5
SELECT * FROM TIPO_TRANSACCION_TARJETA;
...
Hay que identificar las transacciones con cuotas que vencen en el segundo semestre, 
mostrar un resumen con descripción y promedio de montos, y actualizar la tasa 
de interés de ciertos tipos de transacción con una rebaja.
---------------------------------------------------------------------------------------------------------------------------------------------------------------

2.	¿Cuál es la información significativa que necesita para resolver el problema?

La información necesaria es:
Fecha de vencimiento de las cuotas, descripción del tipo de transacción,
monto de las transacciones y tasa de interés del tipo de transacción.
----------------------------------------------------------------------------------------------------------------------------------------------------------------

3.	¿Cuál es el propósito de la solución que se requiere?

El proposito es generar un informe con resumen de transacciones con cuotas que vencen en el 
segundo semestre, ordenado por monto promedio, y actualizar la tasa de interés de ciertos tipos de transacción.
---------------------------------------------------------------------------------------------------------------------------------------------------------------

4.	Detalle los pasos, en lenguaje natural, necesarios para construir la alternativa que usa SUBCONSULTA.

  .  Primero, inserto datos en la tabla seleccion_tipo_transaccion, especifico las columnas fecha, cod_tipo_transac,
     nombre_tipo_transac y monto_promedio donde se insertarán los datos.
     
  .  Utilizo una expresión de tabla común (CTE) llamada TransaccionesFiltradas para organizar mi consulta y hacerla más legible.
 
  .  Dentro de la CTE, selecciono la fecha actual formateada, el código de la transacción, la 
     descripción en mayúsculas y el promedio del monto de la transacción. Para ello, uno las tablas transaccion_tarjeta_cliente,
     tipo_transaccion_tarjeta y cuota_transac_tarjeta_cliente.
     
  .  Filtro las transacciones para incluir solo aquellas con fecha de vencimiento de cuota entre junio y diciembre, que corresponde al segundo semestre del año.
 
  .  Agrupo los resultados por código y nombre del tipo de transacción.
 
  .  Selecciono las columnas que necesito de la CTE TransaccionesFiltradas y ordeno los resultados por el monto promedio de forma ascendente
 
  .  Confirmo los cambios en la base de datos para guardar la inserción de datos.
 
  .  Muestro la tabla seleccion_tipo_transaccion para verificar que los datos se insertaron correctamente
 
  .  Actualizo la tabla tipo_transaccion_tarjeta para rebajar la tasa de interés en un 1%
     y truncar el resultado a 2 decimales. Utilizo una subconsulta con EXISTS para asegurarme de que solo se actualicen las tasas de interés
     de los tipos de transacción que se encuentran en la tabla seleccion_tipo_transaccion.
     
  .  Confirmo los cambios en la base de datos para guardar la actualización de la tasa de interés y muestro la tabla TIPO_TRANSACCION_TARJETA
      para verificar que la tasa de interés se actualizó correctamente
 
 -----------------------------------------------------------------------------------------------------------------------------------------------
 
5.	Detalle los pasos, en lenguaje natural, necesarios para construir la alternativa que usa OPERADOR SET.

  . Primero, obtengo la fecha actual del sistema con la función SYSDATE y la formateo como 'DD-MM-YYYY' utilizando 
     la función TO_CHAR, y le doy el alias 'FECHA' a esta columna.
     
  . Luego, selecciono el código del tipo de transacción desde la tabla tipo_transaccion_tarjeta, a la que le he asignado
     el alias 'tt' y le doy el alias 'CODIGO
  
  . Selecciono el nombre del tipo de transacción, también desde la tabla 'tt', y lo convierto a mayúsculas con la 
     función UPPER. A esta columna le doy el alias 'DESCRIPCION
  
  . Calculo el promedio del monto de las transacciones utilizando la función AVG sobre la columna monto_transaccion de
     la tabla transaccion_tarjeta_cliente, a la que le he asignado el alias 't'. 
     Trunco el resultado a 0 decimales con la función TRUNC y a esta columna le doy el alias 'MONTO_PROMEDIO_TRANSACCION.
     
  . Uno la tabla TRANSACCION_TARJETA_CLIENTE (alias 't') con la tabla TIPO_TRANSACCION_TARJETA (alias 'tt'),
     ademas uno la tabla 't' con la tabla CUOTA_TRANSAC_TARJETA_CLIENTE (alias 'cttc')
  
  . Filtro las transacciones para incluir solo aquellas donde el mes de la fecha de vencimiento de la cuota 
     (fecha_venc_cuota en la tabla 'cttc') esté entre 6 y 12, que corresponden a los meses de junio a diciembre
  
  . Agrupo los resultados por el código y el nombre del tipo de transacción para poder calcular
     el promedio del monto para cada tipo de transacción
  
  . Finalmente, ordeno los resultados por la columna 'MONTO_PROMEDIO_TRANSACCION' de forma ascendente,
     para que los tipos de transacción con menor monto promedio aparezcan primero.
